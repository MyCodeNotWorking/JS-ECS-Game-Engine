    ...
    notify(event, world, payload = undefined) {
        ...
        for (const plugin of this._plugins.keys()) {
            ...
            try {
                ...
                // A throwing plugin hook must not block start()/pause()/stop() for everyone else.
                world.logger?.error(`Plugin threw during "${event}":`, error);
                world.events?.emit('pluginError', { plugin, event, error });
            }
        }
    }
